...
class FerrostarWrapperRepository @Inject constructor(
    ...
    fun setValhallaEndpoint(endpoint: String) {
        walking = FerrostarWrapper(context, RoutingMode.PEDESTRIAN, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
        cycling = FerrostarWrapper(context, RoutingMode.BICYCLE, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
        driving = FerrostarWrapper(context, RoutingMode.AUTO, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
        truck = FerrostarWrapper(context, RoutingMode.TRUCK, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
        motorScooter = FerrostarWrapper(context, RoutingMode.MOTOR_SCOOTER, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
        motorcycle = FerrostarWrapper(context, RoutingMode.MOTORCYCLE, endpoint, androidTtsObserver, appPreferences, routingProfileRepository)
    }
